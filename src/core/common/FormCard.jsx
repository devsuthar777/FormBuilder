...
import { copyToClipboard } from '../commonFunctions/coptyToClipboard';
import toast from 'react-hot-toast';

const FormCard = ({formDetails}) => {
  const [form,setFormDetails] = useState(formDetails);
  

  const shareHandler = () => {
    copyToClipboard(form) ? toast.success("Link Copied!") : toast.success("Can't Copy Link!");
  }
  ...
  return (
    <div  className='w-72 bg-richblue-25  flex flex-col gap-4 hover:scale-105 hover:shadow-lg rounded-lg transition-all duration-200'>
        ...
        <div className='w-full flex justify-around mb-3'>
          <button className='border rounded-md  w-24 hover:shadow-lg text-center' onClick={shareHandler} >Share</button>  
        </div>
